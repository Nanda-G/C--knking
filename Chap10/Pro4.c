...
int main(void)
{
	...
	{
		read_cards(num_in_suits, num_in_rank);
		analyze_hand(num_in_suits, num_in_rank);
		print_result();
	}
}

// read_cards: Reads the cards into external variables; checks for bad cards and duplicate cards;
void read_cards(int num_in_suits[], int num_in_rank[])
{
	...
	while (cards_read < NUM_CARDS)
	{
		...
		while ((ch = getchar()) != '\n') //This allows us to clear the gethar() buffer of the newline character AND makes sure that you can't enter something like 2s2h or 2sjfafads or any other value.
				if (ch != ' ')
					bad_card = true;
...
void analyze_hand(int num_in_suits[], int num_in_rank[])
{
	int num_consec = 0;
	int rank, suit;
	straight = false;
...
void print_result(void)
{
	if (straight && flush && royal_flush == 8) printf("Royal flush");
